...
def generate_pack_name(obj_name, height_label, od_dir):
    """生成pack包名称"""
    od_dir_str = "front" if od_dir == 1 else "rear"
    return f"HY11-{obj_name}-{height_label}-{od_dir_str}"
...
def create_raw_data_sheet(labeled_data):
    ...
    for data, count, phase in labeled_data:
        ...
        # 生成pack包名称
        pack_name = generate_pack_name(data['ObjName'], data['HeightLabel'], data['OdDir'])
...
def analyze_obstacle_data(log_file_path, output_path, output_format='excel', include_raw_data=False):
    ...
    for (obj_name, od_dir, height_label), data_list in obstacle_groups.items():
        ...
        # 生成简洁版结果
        # 生成pack包名称
        pack_name = generate_pack_name(obj_name, height_label, od_dir)
...
if __name__ == "__main__":
    log_file_path = r"D:\PythonProject\data\log_files\3.log"
    ...
    try:
        ...
        if detailed_df is not None and summary_df is not None:
            ...
            if include_raw_data and raw_df is not None:
                ...
                raw_field_descriptions = {
                    'pack包名称': '按照HY11-ObjName-HeightLabel-OdDir格式生成的包名称',
                    '障碍物名称': '障碍物类型名称',
                    '位置': 'front(车前) / rear(车后)',
                    '真实标签': '实际的高低标签',
                    '障碍物ID': 'zhao_od_ID编号',
        ...
        traceback.print_exc()(session_groups),
