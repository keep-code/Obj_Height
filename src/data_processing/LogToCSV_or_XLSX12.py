...
def get_confidence_corrected_status(session_data, height_label):
    """
    获取置信度校对后的检测状态
    只针对真实标签为高的障碍物进行校对
    在1.5米(1500mm)以内，对突然出现置信度为80的数据进行智能校对
    """
    # 先按距离排序（从大到小，模拟接近过程）
    session_data_sorted = sorted(session_data, key=lambda x: x['Dist'], reverse=True)

    # 为每个数据添加置信校对状态，初始值与校对检测状态相同
    for data in session_data_sorted:
        data['ConfidenceCorrectedStatus'] = data['CorrectedHeightStatus']

    # 只对真实标签为高的障碍物进行置信校对
    if height_label != 1:  # 如果不是高障碍物，直接返回，不做任何校对
        return session_data_sorted

    print(f"    调试置信校对：真实标签为高，开始分析置信度80的数据点")

    # 在1.5米以内寻找置信度为80的数据点
    confidence_80_indices = []
    for i, data in enumerate(session_data_sorted):
        if data['Dist'] <= 1500 and data['HeightProb'] == 80:
            confidence_80_indices.append(i)

    print(f"    调试置信校对：找到 {len(confidence_80_indices)} 个置信度为80的数据点")

    # 对每个置信度80的数据点进行校对（重要：从最远的开始处理）
    for i in confidence_80_indices:
        data = session_data_sorted[i]
        print(f"    调试置信校对：分析置信度80数据点 - 距离:{data['Dist']}, 索引:{i}")

        # 找到置信度为80的数据点，检查从这个点开始的模型预测情况
        remaining_data = session_data_sorted[i:]

        # 统计剩余数据中模型预测的情况
        high_predictions = sum(1 for d in remaining_data if d['l_TrainResult'] == 1)
        low_predictions = sum(1 for d in remaining_data if d['l_TrainResult'] == 0)
        total_remaining = len(remaining_data)

        print(
            f"    调试置信校对：剩余数据 {total_remaining} 条，高预测 {high_predictions} 次，低预测 {low_predictions} 次")

        # 显示剩余数据的模型预测序列
        prediction_sequence = [d['l_TrainResult'] for d in remaining_data]
        print(f"    调试置信校对：模型预测序列: {prediction_sequence}")
        ...
        if total_remaining >= 4:
            next_4_predictions = [d['l_TrainResult'] for d in remaining_data[:4]]
            if all(pred == 1 for pred in next_4_predictions):
                should_correct = True
                correction_reasons.append("接下来连续4条模型预测全是高")
                print(f"    调试置信校对：✅ 条件1满足 - 前4次预测: {next_4_predictions}")

        # 条件2：本次检测靠近过程中模型预测全是高
        if high_predictions == total_remaining and total_remaining > 0:
            should_correct = True
            correction_reasons.append("本次检测靠近过程中模型预测全是高")
            print(f"    调试置信校对：✅ 条件2满足 - 全程{total_remaining}次高预测")

        # 条件3：只有不超过2次的低预测
        if low_predictions <= 2:
            should_correct = True
            correction_reasons.append(f"低预测只有{low_predictions}次(<=2次)")
            print(f"    调试置信校对：✅ 条件3满足 - 低预测{low_predictions}次")

        print(f"    调试置信校对：满足的条件: {correction_reasons}")
        print(f"    调试置信校对：是否进行校对: {should_correct}")

        # 如果满足校对条件，将从这个点开始原来报低的数据改为高
        if should_correct:
            corrected_count = 0
            for j in range(i, len(session_data_sorted)):
                current_data = session_data_sorted[j]
                # 如果原来的校对检测状态是低(1)，改为高(2)
                if current_data['CorrectedHeightStatus'] == 1:
                    current_data['ConfidenceCorrectedStatus'] = 2
                    corrected_count += 1
                    print(
                        f"    调试置信校对：校正数据点 - 距离:{current_data['Dist']}, 置信度:{current_data['HeightProb']}, 原状态:低 -> 新状态:高")
                else:
                    print(
                        f"    调试置信校对：保持数据点 - 距离:{current_data['Dist']}, 置信度:{current_data['HeightProb']}, 状态:高(无需校正)")

            print(f"    调试置信校对：从置信度80点(距离{data['Dist']})开始，共校正了 {corrected_count} 个数据点")
        else:
            print(f"    调试置信校对：不满足校对条件，不进行校正")
    ...
    for phase in phases:
        if phase['type'] == 'approach':
            ...
            if len(session_data) >= 5:
                # 对接近会话进行置信度校对，传入height_label参数
                session_data = get_confidence_corrected_status(session_data, height_label)
                approach_sessions.append(session_data)

                # 重要：将校对后的数据更新回原始数据列表
                for j, corrected_data in enumerate(session_data):
                    original_index = phase['start'] + j
                    if original_index < len(all_data):
                        # 将校对后的置信状态复制回原始数据
                        all_data[original_index]['ConfidenceCorrectedStatus'] = corrected_data[
                            'ConfidenceCorrectedStatus']

            # 标记数据（使用已校对的数据）
            for i in range(phase['start'], phase['end'] + 1):
                # 确保数据有置信校对状态
                if 'ConfidenceCorrectedStatus' not in all_data[i]:
                    all_data[i]['ConfidenceCorrectedStatus'] = all_data[i]['CorrectedHeightStatus']
...
def analyze_height_status_changes(session_data, use_confidence_corrected=False):
    ...
    # 选择使用哪种状态进行分析
    status_key = 'ConfidenceCorrectedStatus' if use_confidence_corrected else 'CorrectedHeightStatus'
    current_status = session_data[0][status_key]
    change_start_idx = 0

    for i, data in enumerate(session_data):
        if data[status_key] != current_status:
            changes.append({
            ...
            current_status = data[status_key]
            change_start_idx = i
...
def create_raw_data_sheet(labeled_data):
    """创建原始数据表，显示每条数据的分组情况，增加校对检测状态列和置信校对列"""
    raw_results = []
    ...
    for data, count, phase in labeled_data:
        ...
        # 获取校对后的检测状态
        corrected_status = get_corrected_height_status(data['HeightStatus'], data['PosDeDis1'])
        corrected_status_str = "高" if corrected_status == 2 else "低"  # 校对检测状态

        # 确保数据中有置信校对状态，如果没有则使用校对检测状态
        if 'ConfidenceCorrectedStatus' not in data:
            data['ConfidenceCorrectedStatus'] = corrected_status

        # 获取置信校对后的检测状态
        confidence_corrected_status = data['ConfidenceCorrectedStatus']
        confidence_corrected_status_str = "高" if confidence_corrected_status == 2 else "低"  # 置信校对状态
        ...
        # 基于置信校对后的状态判断是否正确
        is_correct = ((data['HeightLabel'] == 0 and confidence_corrected_status == 1) or
                      (data['HeightLabel'] == 1 and confidence_corrected_status == 2))
        ...
        raw_results.append({
            ...
            '校对检测状态': corrected_status_str,  # 雷达距离校对后的检测状态
            '置信校对': confidence_corrected_status_str,  # 置信度校对后的检测状态
            '置信度': data['HeightProb'],
            '模型预测': "高" if data['l_TrainResult'] == 1 else "低",  # 显示为文字
            '模型概率': data['probability'],
            '检测阶段': phase_str,
            '是否正确': "✓" if is_correct else "✗"  # 基于置信校对后状态判断
        })
...
def analyze_obstacle_data(log_file_path, output_path, output_format='excel', include_raw_data=False):
    ...
    print(f"🔧 校对检测状态规则: 雷达距离<=500时，检测状态设为'高'")
    print(f"🔧 置信校对规则: 1.5米内置信度80数据的智能校对")
    ...
    for (obj_name, od_dir, height_label), data_list in obstacle_groups.items():
        ...
        for zhao_od_id, id_data_with_session in id_groups.items():
            ...
            for session_idx, session_data in session_groups.items():
                # 基于置信校对后的状态判断是否正确
                session_correct = all(
                    (height_label == 0 and data.get('ConfidenceCorrectedStatus',
                                                    get_corrected_height_status(data['HeightStatus'],
                                                                                data['PosDeDis1'])) == 1) or
                    (height_label == 1 and data.get('ConfidenceCorrectedStatus',
                                                    get_corrected_height_status(data['HeightStatus'],
                                                                                data['PosDeDis1'])) == 2)
            ...
            if all_sessions_correct:
                ...
                for session_idx in incorrect_sessions:
                    ...
                    # 重要：基于置信校对后的状态进行分析
                    changes = analyze_height_status_changes(session_data, use_confidence_corrected=True)
                    ...
                    detailed_results.append({
                        ...
                        '首次错误距离(mm)': str(first_error_dist) if first_error_dist > 0 else "-1",
                        '最近错误距离(mm)': str(latest_error_dist) if latest_error_dist > 0 else "-1",
                        '错误详情': error_desc
                    })
        ...
        if not obstacle_has_any_error:
            ...
            summary_results.append({
                ...
                '距离范围(mm)': f"{max(all_distances)}~{min(all_distances)}",
                '检测结果': f"✗ {error_type}",
                '最大错误距离(mm)': str(max_error_dist) if max_error_dist > 0 else "-1",
                '最小错误距离(mm)': str(min_error_dist) if min_error_dist > 0 else "-1",
                '备注': remark
            })
    ...
    if include_raw_data:
        ...
        # 调试：检查置信校对的效果
        if len(raw_df) > 0:
            confidence_80_data = raw_df[raw_df['置信度'] == 80]
            if len(confidence_80_data) > 0:
                print(f"📊 调试置信校对效果:")
                print(f"  - 置信度为80的数据: {len(confidence_80_data)} 条")
                high_obstacles_80 = confidence_80_data[confidence_80_data['真实标签'] == '高障碍物']
                if len(high_obstacles_80) > 0:
                    print(f"  - 其中高障碍物: {len(high_obstacles_80)} 条")
                    within_1500_high = high_obstacles_80[high_obstacles_80['距离Dist'] <= 1500]
                    if len(within_1500_high) > 0:
                        print(f"  - 1.5米内高障碍物置信度80: {len(within_1500_high)} 条")
                        corrected_count = len(within_1500_high[within_1500_high['置信校对'] == '高'])
                        print(f"  - 被校正为高的数量: {corrected_count} 条")
                        print(f"  - 校正率: {corrected_count / len(within_1500_high) * 100:.1f}%")

                        # 显示前几条数据的详细信息
                        print(f"  - 前5条1.5米内高障碍物置信度80数据:")
                        for idx, row in within_1500_high.head(5).iterrows():
                            model_pred = row['模型预测']
                            corrected_status = row['校对检测状态']
                            confidence_status = row['置信校对']
                            is_corrected = "✅校正" if corrected_status != confidence_status else "⭕保持"
                            print(
                                f"    距离:{row['距离Dist']}, 校对检测状态:{corrected_status}, 置信校对:{confidence_status}, 模型预测:{model_pred}, {is_corrected}")

                        # 检查是否还有连续报低的情况
                        consecutive_low_groups = []
                        current_group = []
                        for idx, row in within_1500_high.iterrows():
                            if row['置信校对'] == '低':
                                current_group.append(row)
                            else:
                                if len(current_group) >= 4:  # 连续4次或更多报低
                                    consecutive_low_groups.append(current_group)
                                current_group = []

                        if len(current_group) >= 4:  # 处理最后一组
                            consecutive_low_groups.append(current_group)

                        if consecutive_low_groups:
                            print(f"  ⚠️  警告：仍存在连续4次以上报低的情况:")
                            for i, group in enumerate(consecutive_low_groups):
                                print(
                                    f"    组{i + 1}: {len(group)}次连续报低，距离范围{group[0]['距离Dist']}-{group[-1]['距离Dist']}")
                                for row in group:
                                    print(
                                        f"      距离:{row['距离Dist']}, 置信度:{row['置信度']}, 模型预测:{row['模型预测']}, 校对检测状态:{row['校对检测状态']}, 置信校对:{row['置信校对']}")
                        else:
                            print(f"  ✅ 没有连续4次以上报低的情况")
            else:
                print(f"📊 没有发现置信度为80的数据")
    else:
        raw_df = None
    ...
    print("-" * 50)
    print("🔧 详细分析表优化:")
    print("  • 已移除'备注'和'距离范围(mm)'列，使表格更简洁")
    print("  • 检测状态基于置信校对后的结果判断")

    print("\n🔧 置信校对算法:")
    print("-" * 50)
    print("🔧 智能校对规则:")
    print("  • 仅针对真实标签为高的障碍物进行校对")
    print("  • 对于真实标签为低的障碍物，保持原状不做处理")
    print("  • 在1.5米(1500mm)以内寻找置信度为80的数据点")
    print("  • 校对条件(满足任一即可):")
    print("    - 接下来连续4条模型预测全是高")
    print("    - 本次接近过程中模型预测全是高")
    print("    - 模型预测中低预测不超过2次")
    print("  • 满足条件时：将从置信度80数据开始，原报低改为报高")

    print("\n🔧 原始数据分组表新增:")
    print("-" * 50)
    print("🔧 新增'置信校对'列:")
    print("  • 位置：'校对检测状态'列右侧")
    print("  • 内容：经过置信度智能校对后的最终检测状态")
    print("  • 模型预测列现在显示为'高'/'低'文字")
    print("  • '是否正确'基于置信校对后的状态判断")

    print("\n💡 核心改进:")
    print("-" * 50)
    print("🔧 避免高障碍物置信度重置错误:")
    print("  • 仅针对真实标签为高的障碍物进行置信校对")
    print("  • 低障碍物保持原有检测逻辑，不做任何校对")
    print("  • 针对1.5米内突然出现置信度80的情况")
    print("  • 基于模型预测趋势进行智能校对")
    print("  • 减少因置信度重置导致的高障碍物误报低")
    print("  • 提高近距离高障碍物检测的准确性")
...
if __name__ == "__main__":
    log_file_path = r"D:\PythonProject\data\log_files\145_174.log"
    output_path = r"D:\PythonProject\data\csv_files\5\145_174"
    ...
    try:
        ...
        if detailed_df is not None and summary_df is not None:
            ...
            field_descriptions = {
                ...
                '测试次数说明': '当前记录对应的测试次数',
                '数据点总数': '数据记录条数',
                '检测状态': '全程正确/存在报高/存在报低(基于置信校对后状态)',
                '首次错误距离(mm)': '第一次出现错误时的距离(基于置信校对后状态)',
                '最近错误距离(mm)': '最后一次出现错误时的距离(基于置信校对后状态)',
                '错误详情': '错误的具体距离区间信息(基于置信校对后状态)'
            }
            ...
            summary_field_descriptions = {
                ...
                '检测结果': '✓全程正确 / ✗存在报高 / ✗存在报低(基于置信校对)',
                '最大错误距离(mm)': '所有测试中首次错误距离的最大值',
                '最小错误距离(mm)': '所有测试中最近错误距离的最小值',
                '备注': '标准化格式的测试结果描述'
            }
            ...
            if include_raw_data and raw_df is not None:
                ...
                raw_field_descriptions = {
                    ...
                    '雷达距离': '雷达直接回波距离(已过滤0值)',
                    '检测状态': '系统报告的原始高低状态',
                    '校对检测状态': '基于雷达距离校对后的高低状态(<=500时为高)',
                    '置信校对': '经过置信度智能校对后的最终检测状态',
                    '置信度': '系统置信度',
                    '模型预测': '模型预测结果(高/低)',
                    '模型概率': '模型预测概率值',
                    '检测阶段': '第几次接近或远离过程',
                    '是否正确': '置信校对后检测结果是否与真实标签一致'
                }
