...
def format_distance_ranges(changes, height_label):
    ...
    # 改进：检查是否存在从某个距离开始全程正确的情况
    if len(changes) >= 2:
        # 找到最后一个正确的连续段
        last_correct_segment = None
        for i in range(len(changes) - 1, -1, -1):
            ...
            if is_correct:
                ...
                for j in range(i + 1, len(changes)):
                    ...
                    next_is_correct = (height_label == 0 and next_change['status'] == 1) or (
                                height_label == 1 and next_change['status'] == 2)
                    if not next_is_correct:
                        is_final_segment = False
                        break

                if is_final_segment and change['end_dist'] < change['start_dist'] * 0.8:  # 放宽条件到80%
                    last_correct_segment = change
                    break

        # 或者检查是否有明显的距离分界点
        if last_correct_segment is None and len(changes) >= 2:
            # 寻找错误转为正确的转折点
            for i in range(len(changes) - 1):
                current_change = changes[i]
                next_change = changes[i + 1]

                current_is_correct = (height_label == 0 and current_change['status'] == 1) or (
                            height_label == 1 and current_change['status'] == 2)
                next_is_correct = (height_label == 0 and next_change['status'] == 1) or (
                            height_label == 1 and next_change['status'] == 2)

                # 如果从错误转为正确，且后续都正确
                if not current_is_correct and next_is_correct:
                    # 检查后续是否都正确
                    all_subsequent_correct = True
                    for j in range(i + 1, len(changes)):
                        subsequent_change = changes[j]
                        subsequent_is_correct = (height_label == 0 and subsequent_change['status'] == 1) or (
                                    height_label == 1 and subsequent_change['status'] == 2)
                        if not subsequent_is_correct:
                            all_subsequent_correct = False
                            break

                    if all_subsequent_correct and next_change['start_dist'] < current_change['end_dist'] * 0.9:
                        last_correct_segment = next_change
                        break

        if last_correct_segment:
            expected_status = "报低" if height_label == 0 else "报高"
            descriptions.append(f"Dist<{last_correct_segment['start_dist']}后全程{expected_status}")
...
def create_raw_data_sheet(labeled_data):
    ...
    for data, count, phase in labeled_data:
        ...
        height_status_str = "高" if data['HeightStatus'] == 2 else "低"  # 去掉"报"字
...
if __name__ == "__main__":
    ...
    output_path = r"D:\PythonProject\data\csv_files\3"
