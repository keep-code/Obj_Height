...
def analyze_obstacle_data(log_file_path, output_path, output_format='excel', include_raw_data=False):
    ...
    for (obj_name, od_dir, height_label), data_list in obstacle_groups.items():
        ...
        for zhao_od_id, id_data_with_session in id_groups.items():
            ...
            for session_idx, session_data in session_groups.items():
                # 基于校对后的状态判断是否正确
                session_correct = all(
                    (height_label == 0 and get_corrected_height_status(data['HeightStatus'], data['PosDeDis1']) == 1) or
                    (height_label == 1 and get_corrected_height_status(data['HeightStatus'], data['PosDeDis1']) == 2)
        ...
        if not obstacle_has_any_error:
            ...
            summary_results.append({
                ...
                '备注': f"所有ID({','.join(map(str, sorted(id_groups.keys())))})全程检测正确(基于校对后状态)"
            })
        else:
    ...
    print("-" * 50)
    print("🔧 核心改进:")
    print("  • 新增'校对检测状态'列：当雷达距离<=500时，检测状态自动校正为'高'")
    print("  • 所有算法分析基于校对后的状态，而非原始检测状态")
    print("  • 原始数据分组表同时显示原始和校对后的检测状态")
    print("  • '是否正确'判断基于校对后的检测状态")
    print("\n📊 校对规则:")
    print("  • 当雷达距离(PosDeDis1) <= 500mm时：校对检测状态 = '高'")
    print("  • 当雷达距离(PosDeDis1) > 500mm时：校对检测状态 = 原始检测状态")
    print("\n🔄 算法修改范围:")
    print("  • identify_sessions_by_distance_only(): 使用校对后状态")
    print("  • analyze_height_status_changes(): 使用校对后状态")
    print("  • get_latest_error_distance(): 使用校对后状态")
    print("  • format_distance_ranges(): 使用校对后状态")
    print("  • get_status_analysis(): 使用校对后状态")
    print("  • create_raw_data_sheet(): 显示原始和校对后状态")
    print("  • analyze_obstacle_data(): 正确性判断使用校对后状态")
    ...
    print("-" * 50)
    print("🔧 距离计算逻辑:")
    print("  • 首次错误距离(mm): 第一个错误段的起始距离")
    print("    - 找到第一个与真实标签不一致的检测段")
    print("    - 记录该错误段开始时的距离(最远端)")
    print("  • 最近错误距离(mm): 最后一个错误段的结束距离")
    print("    - 找到最后一个与真实标签不一致的检测段")
    print("    - 记录该错误段结束时的距离(最近端)")
    print("  • 为什么两者不同:")
    print("    - 首次错误距离反映问题首次出现的位置")
    print("    - 最近错误距离反映最后检测错误的位置")
    print("    - 两者之间可能有正确检测的区间")

    print("\n🆕 新增功能说明:")
    print("-" * 50)
    print("🔧 详细分析表新增列:")
    print("  • 最近错误距离(mm): 每次检测中最后一次出现错误的距离(基于校对后状态)")
    print("    - 有错误时显示具体距离值(错误段的结束距离)")
    print("    - 无错误时显示 '-1'")
    print("\n📊 简洁汇总表新增列:")
    print("  • 最小错误距离(mm): 该障碍物所有有错误检测中的最小错误距离(基于校对后状态)")
    print("    - 计算来源于详细分析表中大于0的最近错误距离")
    print("    - 无错误时显示 '-1'")
    print("\n📋 原始数据分组表新增列:")
    print("  • 校对检测状态: 基于雷达距离校对后的检测状态")
    print("    - 雷达距离<=500时显示'高'，否则显示原始状态")
    print("  • 是否正确: 基于校对后状态与真实标签的对比结果")
...
if __name__ == "__main__":
    ...
    output_path = r"D:\PythonProject\data\csv_files\1\1_40"
