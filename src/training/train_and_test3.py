...
class ObstacleHeightClassifier:
    ...
    def save_misclassified_samples(self, original_df):
        """保存分类错误的样本"""
        ...
        # 保存到CSV文件
        output_file = 'misclassified_samples.csv'
...
def main():
    ...
    # 1. 加载和预处理训练数据
    train_df = classifier.load_and_preprocess_data('train_group2.csv')

    # 2. 特征工程
    train_df_processed = classifier.feature_engineering(train_df)

    # 3. 训练模型（可以设置test_size=0使用全部数据训练）
    X_test, y_test, y_pred, y_pred_proba = classifier.train_model(
        train_df_processed, test_size=0.0, random_state=42
    )

    # 4. 预测测试数据（如果有新的测试文件）
    # 注意：这里假设测试文件名为 'Test_OD.csv'，请根据实际情况修改
    try:
        test_results = classifier.predict_test_data('train_group1.csv', 'Prediction_Results.csv')
        print("\n预测完成！")
